...
- 回答：从这几个维度去排查吧，比如说 CPU 方面的话，可以 `vmstat` 去看一个整体的 CPU 使用情况，可以通过`vmstat 1 5`收集一个时间段的数据，比如每一秒刷新收集五秒。然后也可以通过 htop或者top 去看，它上面也会显示整体的一个 CPU和内存的整体使用情况。那如果要看内存的话，常用就是用 free，HTOP/Top 也可以。刚刚提到的那 vmstat，通过看swap次数，buff和cache也能看到当前内存的使用情况。那除此之外，看磁盘 IO 的话，HTOP 也可以看一个整体的IO情况，然后通常还会用 iostat 去看具体某个磁盘的平均的 IOPS，单次写，读的大小等等。然后如果要某个进程的这个 CPU，memory 使用情况，可以用 HTOP以及用 PS 去看一个进程的 PID，user 等等的更多信息。
    ...
    **2.1** 若top命令后看到一个进程使用率为100%，如何分析问题，在什么情况下会出现这种情况
    - 查看**系统日志**/var/log/syslog 或 /var/log/messages，判断是否有相关的错误信息，比如死循环，或者本身代码算法比较低效，高并发处理等等。如果没有，可能就是一个cpu密集型的任务在执行，并不是问题，比如可能是3d渲染，比特币挖矿或者模型训练。
    - 看htop中该进程的cpu,主要看**用户态和系统态时间**，系统态时间高可能是锁竞争或者大量I/O操作，内存(RES物理内存和VIRT虚拟内存)的使用情况。以及通过看r判断cpu的饱和度，如果r很高(> CPU 核心数) → CPU 不够。
...
- 回答：首先进程是操作系统的一次执行过程，那它也是系统进行资源分配和调度的基本单位，线程则是进程中的一个执行单元，也是 CPU 的调度和分配的基本单位。然后在资源方面，每个进程有独立的内存空间和系统资源，那线程则是共享进程的内存空间和这个系统资源，那再到就是调度和执行，进程和线程都是通过系统内核去调度和执行，但是进程会比线程调度的成本更低，因为他不会涉及到切换内存。那再就是关于通信的话，进程和进程之间通信是通过像管道、消息队列或者是信号以及共享内存等机制。线程的话则是直接通过它的一个共享变量进行通信，因为他们相当于是共享现进程的一个内存。然后。最后就是健壮性，那进程，它通常健壮性会更强，因为它单个进程崩溃不会导致其他进程崩溃，但线程则健壮性比较差，因为单个线程的崩溃很可能会导个导致整个进程的崩溃。
    ...
    **5.3** 进程如何通信？
    ...
    - 共享内存（Shared Memory）
        - 数据库（MySQL、PostgreSQL）让**多个进程通过 “内存映射” 将其映射到各自的虚拟地址空间**，进程直接读写这块内存（无需内核中转）
    - 信号（Signal）
        - 底层：kill()实现。
    ...
    **5.4** linux中的通道是什么？
    ...
    - 回答：首先进程之间主要是通过这几种方式去通信，像常见的通过管道，那管道可能又分为无名管道和命名管道，底层基于pipe()和mkfifo()函数实现的。那通常我们用无名管道会用的多一些，本质是 **shell 创建的父子进程间通信**，具体就是通过一条竖线，那你一个命令将输入这个管道当中，那通过这个管道又输出到这个下一个命令。那当然还可以通过命名管道，那你就创建一个 fifo 文件，可以让两个独立进程之间进行通信。然后可以通过信号通信，通常是传递**事件而不是数据**，那常见的信号比如通过kill发送SIGKILL或者SIGTERM，以及键盘crtl C发送SIGINT信号去杀死进程。那再就是可以通过像消息队列进行通信，消息队列分为**内核级和应用层的中间件**，我们现在可以通过像 Kafka 或者 rockmq 等中间件去实现两个进程之间通过中间件去实现通信，或者内核级通过**msgsnd/msgrcv**系统调用。还有通过共享内存进行通信，也是效率最高的内核级的通信方式，通常让**多个进程通过 “内存映射” 将其映射到各自的虚拟地址空间**，进程直接读写这块内存（无需内核中转）。那再就是套接词socket，比如基于TCP/UDP网络传输的socket或者是**本地UNIX套接词**，都是底层通过调用底层的 read and write 函数去在这个 socket 套接字中去进行读写操作去进行通信。
...
- 回答：首先当用户在终端输入了一行命令后，那么终端程序比如bash程序，那它就会去分析用户这行命令哪些是命令本身，哪些是选项，哪些是参数，然后它会去分辨这个命令，去判断它到底是一个内部命令还是外部命令，还是一个外部别名。那针对不同的命令，它会有不同处理方式。如果是内部 命令，shell会直接调用内部的函数来执行该命令，那如果是外部 命令，它就会去查看当前的这个环境变量的 path 路径，去具体查看这个外部命令的可执行文件，然后它会在系统中通过 fork 创建一个子进程，然后通过 exec 去执行这个可执行文件。那么关于别名，它就会去找到对应别名的命令，将这个命令替换掉别名再进行操作。那么除此之外，它还会去进行一些替换，比如说命令中会有一些变量，它会去查找这个变量，并将变量的实际内容替换成变量本身。那再就是，还有就是可能会有一些通配符，你比如说 find 命令搭配一个通配符星号的.*TXT，那么它就会将这个通背符换成当前目录下所有以 txt结尾的文件放进来执行。那还有就是如果说他通过括号去执行另一个命令，那他比如说像 Echo 括号一个命令 date，那他就会将括号中这个命令先执行完，然后替换成这个命令，然后再执行诊断命令，那最后就是对输入输出的处理，包括有重定向，管道，以及标准输入输出和错误输出等等。那最后它会也会生成相应的命令执行结果，你可以通过 Echo 问号去查看上一个命令执行成功与否。
...
- 回答：首先第一个括号通常是用来**创建子shell执行命令**，那括号中可能比如说常见的 Echo 括号 date，那它就会将 date 先在括号中执行，然后将括号中 date 执行的结果通过 Echo 输出到屏幕上。那再就是双括号通常是用于**数值计算**或者**数值逻辑判断**，那比如说括号中可以有两个变量的加减乘除。花括号通常是用来**给变量边界界定**，**批量生成内容**或者**通过父进程执行命令**，比如`echo ${var}file`确定变量是var而不是varfile，比如`touch file{1..3}.txt`批量生成生成file1.txt、file2.txt、file3.txt。那在就是中括号通常是旧的条件测试，等价于test。那双括号就是现在比较新的一个条件测试，它支持正则表达式，而且不兼容sh。
...
- locate(推荐locate)：locate -i "readme.md"

- 回答：比较常见的就是 find 和 locate，那通常我个人建议用 locate DB，因为它通过数据库的方式会相比find去在磁盘中整个遍历会快得多。locate通常就 locate -i 接文件名，那它就会在数据库中快速找到这个具体的文件，那通常如果你文件发生了一些改变，那你是需要进行 updatedb 的操作，去定期去更新数据库。那么find实时遍历指定路径下的所有文件 / 目录，支持多条件筛选，速度慢但灵活，那么 fine 通常它的一个命令的一个逻辑就是 find 接路径名、选项和操作。那么比如常见的像 find，你接一个当前的一个路径，然后再接-name，然后接一个文件名。
    ...
    **9.1** 如何查看文件或者目录大小？
    ...
    - 回答：查看文件或者目录的大小，首先你可以通过 ls -l 查看当前目录下所有的文件的大小，那再就是也可以通过 du -sh 去查看某个文件或者目录的具体的大小，那在就是可以通过 df -h 看磁盘的大小,当然这个不是直接看文件和目录，那最后还可以通过 stat 去看一个文件的大小，它当然也会有文件的更具体的一些属性，包括最后访问时间，最后修改时间等。
    ...
    **8.1** du和df的区别？
    ...
    - 回答：首先第 du 它主要是看一个文件和目录的磁盘使用情况，通常搭配 du - sh 去看目录或者文件它的一个大小。那 DF 它主要是看文件系统的磁盘空间使用情况。你通过 DF -h 可以看到所有挂载点的磁盘使用情况。
...
- 回答：通常我们可以用 grep 和 awk 命令，那主要 grep 就是通过**匿名管道去实现**，比如说你先读取一个文件，然后加上管道，然后 grep 一个关键词去查找有这个关键词的行数，并打印出来。那 awk就会更精准一点，比如说你想只打印这个输出中**某某一列或者某几列**，那你可以搭配这个 **print**，加上对应的这个列数，你还可以通过**改变分隔符**，比如说你想通过逗号分隔，也可以 **-F** 加上这个逗号。那也可以**按条件过滤**，比如说第二列是数字，那你可以通过awk把第二列所有大于2的这个行显示出来，那你可以在前面加上这个行数大于2。


    **12.1** 在Linux上如何用命令查看日志文件里面“error”的个数，并且统计行数?
...
- 回答：shell脚本就是通过**自动化的方式去执行一系列命令的语句的文本文件**，那么最常见的shell比如是bash 会更多一点，那这个需要脚本中除了相关的这个命令以外，还会有一些像变量，还有条件语句、循环等编程的结构。

    **16.1** shell脚本第一行是什么？
    - 定义：在shell脚本的第一行通常会包含一个特殊的注释行，通常以#!/bin/bash开头，**称为shebang（#!），用于指定脚本的解释器**。

16. 在服务器上仅开启22端口和443端口，其他端口全部禁止访问，使用iptables如何实现。
...
- 回答：首先用户态是在**应用程序运行的模式，CPU会执行应用程序本身的代码**，那内核态则是**操作系统内核运行的模式，CPU 会切换到内核态去执行一些内核中的命令**，那通常在这几种情况下会发生用户态和内核态的切换。比如用户态代码中会涉及到一些去访问硬件资源或者是内核资源的情况，比如就常见的一个应用程序需要去读取或者是写入文件到磁盘中，那此时**用户态程序需要执行系统调用**， CPU 就会**切换到内核态**，去执行内核中的内部函数。那么还有一种情况是当我们的一些**硬件设备去发送了信号给这个 CPU**，比如说中断的信号，那这个时候 CPU 也会强制 就是**切换成内核态去执行这个信号**。比如常见的可能我们在键盘上按下 Ctrl c，那此时 CPU 就会切换到内核去杀死当前这个进程。那除此之外还会有一些**应用程序执行发生的异常**，比如在用户态执行一个应用程序代码中出现了一些除零异常，或者非法内存访问等的异常。那此时 CPU 也会强制切换成内核态去杀死当前进程。那么在用户态切换成进程态的时候，那通常 CPU 会将用户态上下文存到内存栈当中，并且在后续执行中会去调用这个内核栈当中的数据。当然内核执行完任务后，从**内核栈中读取之前保存的用户态上下文**，恢复到 CPU 寄存器中，再将栈指针切回用户栈，应用程序从暂停位置继续执行。


18. 解释linux中万物皆为文件是什么意思？
...
- 回答：首先每个文件都会有对应**唯一的 inode**，存储资源的核心元数据。并且进程并不会直接操作文件的inode，而是通过**文件描述符**简介操作。常见的比如像一些**文本文件、二进制文件、配置文件**，那这些 Windows 系统上也会有，那这种文件就是普通的一些文件，那还有包括 Linux 上的**目录**，其实也是一种特殊的文件，那它主要存储的是它目录下的一些一些文件或者子目录他们的一些元数据包括文件名、属性，还有文件数据的那个数据块的指针等等。那再就是像一些**硬件设备**，它在 Linux 中也是以文件的方式呈现的，主要是分为像**字符设备文件和块设备文件**，像字符设备文件比如接口或者串**允许字节流方式访问**，那像**块设备文件**比如说一些 u 盘、硬盘**允许像访问块方式访问**。还有比较常见的像管道和 socket。 那管道它分为**匿名管道和匿名管道**，那通常情况下管道其实本质上它也就像一个文件一样，那通常一个进程会将自己的数据输入到这个管道文件中，那另一个进程会通过读取这个管道文件中的数据，那实现两个进程之间的通信。那还有就是像 SOCKET，那 SOCKET 可能更多，大家理解的是网络传输中的网络套接字，基于 TCP UDP 的一个网络 socket，那通常网络 socket 它其实也是基于本地的 socket 函数去创建，然后会生成一个**文件描述符**，那两个进程也是通过这个文件描述符实现一个通信，当然本地进程也可以通过 **Unix 套接字**去实现通信，也是一样的道理。那最后还有像一些**符号链接**，那其实符号链接本身它也是一个特殊的文件，它主要是存保存**另一个文件或目录的路径信息**。
...
20. 什么是inode？
- 定义：**inode（索引节点）**是文件系统中的一个**数据结构**，用于**存储文件的元数据（metadata）**，对于普通文件，会存储**权限、所有者、文件大小、创建时间、修改时间等信息**，以及指向**文件数据块**的指针。对于目录文件，会存储**目录属性的数据块**，其**数据块**会保存**文件名→inode 号的映射**，对于设备文件，会存储**设备号（主设备号 + 次设备号）**。

- 答案：inode 的是相当于 Linux 中所有文件的一个身份证，它存储了文件本身的**源数据 Metadata**。 那针对不同的文件，它们 inode 的也大不相同，比如像普通的文本文件或者二进制文件或者是配置文件这些，那他们的 inode 的就是会保存他们的文件的**权限、所有者、文件大小、创建时间、修改时间等信息数据以及存储数据的快block的指针**。那么针对目录的话，目录本身也是一个特殊的文件，那目录的 **inode 的会存储目录本身它的这个数据块的指针**，那他的数据块当中会存储这些他**目录下文件名对应他们 inode 的一个映射表**。那么针对像一些硬件设备，他们这些文件会的，**inode 会存储他们的硬件设备号**，主要有一个主设备号和次设备号。



21. 在Linux系统中如何查看僵尸进程？
- 通过ps命令查看僵尸进程：

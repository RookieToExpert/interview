...
| 服务类型                | Azure 并发支持                   | AWS 并发支持                            | 阿里云 并发支持                          |
|-------------------------|----------------------------------|------------------------------------------|-------------------------------------------|
| **虚拟机（VM / ECS）**    | ✅ 完全自定义并发，取决于配置和服务设计 | ✅ 完全自定义并发，实例可开多少线程都行        | ✅ 完全自定义并发，按资源配置和内核数扩展      |
| **容器实例（ACI / Fargate / ECI）** | ❌ 单实例不支持并发，请求串行（一个实例一个请求） | ❌ 单实例默认不支持并发（每次运行一个任务）     | ❌ 单实例不支持并发（函数式容器场景，临时任务）  |
| **容器服务（Container App / App Runner / SAE）** | ✅ 支持并发（每个实例可处理多个请求）     | ✅ 支持并发，默认最多同时处理 100 请求/实例       | ✅ 支持并发，轻量级应用每个 Pod 默认支持多请求    |
| **Web 应用（App Service / Elastic Beanstalk / 函数计算 Web）** | ✅ 支持并发，自动扩展线程池和连接数     | ✅ 支持并发，根据负载自动扩展实例和并发量       | ✅ 支持并发，自动扩展并发连接                |
| **Kubernetes 服务（AKS / EKS / ACK）** | ✅ 支持并发，取决于容器/Pod配置          | ✅ 支持并发，K8s层自行控制资源调度            | ✅ 支持并发，最大由 Pod 副本数和服务设计决定     |
| **Batch 服务（Azure Batch / AWS Batch / 阿里云批处理）** | ✅ 支持大规模并发任务（百万级）         | ✅ 支持大规模并发任务处理，配合 Spot 扩展       | ✅ 支持分布式批处理并发，可自动调度计算资源     |
| **函数服务（Function / Lambda / 函数计算）** | ✅ 每个实例支持并发（默认最多 100 / 实例）| ❌ 单实例不并发，**默认单请求处理一个实例**     | ✅ 支持并发（默认实例 100 并发，函数层配置）     |

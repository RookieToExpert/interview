...
| 批处理 / 高并发任务    | Azure Batch                          | AWS Batch                          | Batch Compute / 计算任务调度平台     | 适合大规模仿真、渲染、模型训练、高并发无状态任务                                                                  |
| Serverless 函数服务   | Azure Functions                      | AWS Lambda                         | 函数计算（Function Compute）        | 无服务器函数计算，适合事件驱动、Webhook、自动任务、轻量异步处理                                                   |

...
在之前的一个制造业客户项目中，我们主要负责他们的核心业务系统上云，包括 MES、ERP 和部分 SCADA 系统。整体设计思路是 业务层和平台层分开：
在业务层，MES 和 ERP 的应用通过 rehost 方式上 VM，数据库通过 DMS 迁移到云上的托管 SQL；SCADA 的实时控制仍保留在本地工厂，保证毫秒级延迟，而数据采集和分析部分迁到云端运行。
在平台层，我们采用 Hub-Spoke 架构 作为网络底座，通过 Terraform 统一部署和管控。全局入口使用 Azure Front Door 做全球加速和 WAF 防护，区域内由 Application Gateway 做 TLS 终结和路径路由。Hub 层集中放置 ExpressRoute Gateway 与本地互通，并配合 Azure Firewall 和 Private Endpoint，把数据面全部私有化。这样既能保证跨区域容灾，又能满足合规和审计要求。

在迁移流程上，我们分为三个阶段：
评估：通过 Azure Migrate 工具收集本地虚机的 CPU、磁盘 IO 等基线数据，并结合兼容性检查，产出推荐 SKU 和迁移顺序。
复制：先做全量，再通过 hypervisor 级别的快照和日志跟踪实现增量同步，保证一致性。
测试与割接：我们先搭建影子环境进行验证，最后采用蓝绿/金丝雀的方式切流量。数据库割接时，通过专线完成最终增量复制，停机窗口大概在 25 分钟以内，并且保留回滚方案。
最终客户的收益非常明显：工厂端的实时控制没有受到影响，云端的 ERP 和 MES 获得了托管数据库的高可用与自动容灾；网络和安全策略也实现了集中管理，合规审计更容易通过。更重要的是，整个迁移过程风险可控，有影子环境和回滚路径，客户的业务连续性得到了保障。

...
-- 基础索引
CREATE INDEX ON users(created_at);                    -- 参见㊶
CREATE INDEX ON products((lower(name)));              -- 参见㊶㊷
CREATE INDEX ON orders(status);                       -- 参见㊶

-- JSON 表达式索引
CREATE INDEX ON orders((extra->>'channel'));          -- 参见㊶㊸
CREATE INDEX ON orders(((extra->>'vip')::bool));      -- 参见㊶㊸㊹
```
## 批量造数
-- 2万用户
INSERT INTO users(email,name,created_at)
SELECT 'user'||i||'@ex.com', 'User '||i,
       now() - (random()*180||' days')::interval       -- 参见㊻㊵
FROM generate_series(1,20000) i;                       -- 参见㊺

-- 3000 商品
INSERT INTO products(sku,name,price,tags,created_at)
SELECT 'SKU'||i, 'Product '||i,
       round((random()*500+5)::numeric,2),             -- 参见㊻⑤①（见术语表注 51）
       ARRAY['cat_'||((i%10)+1),                       -- 参见㊽
             CASE WHEN random()<0.2 THEN 'hot' ELSE 'norm' END],
       now() - (random()*180||' days')::interval       -- 参见㊻㊵
FROM generate_series(1,3000) i;                        -- 参见㊺

-- 8万订单（含 JSONB 随机字段）
INSERT INTO orders(user_id,status,total,extra,created_at)
SELECT
  (SELECT id FROM users OFFSET floor(random()*20000) LIMIT 1),  -- 参见㊼㊻
  (ARRAY['created','paid','shipped','cancelled'])[floor(random()*4)+1], -- 参见㊽
  round((random()*200+10)::numeric,2),                          -- 参见㊻⑤①
  jsonb_build_object('channel',                                 -- 参见㊾
    (ARRAY['web','app','mini','partner'])[floor(random()*4)+1], -- 参见㊽
    'vip', (random()<0.25),
    'note', to_jsonb(md5(random()::text))),                     -- 参见㊿
  date_trunc('day', now() - (random()*150||' days')::interval)  -- 参见㊱㊻㊵
    + (random()*23||' hours')::interval                         -- 参见㊻㊵
FROM generate_series(1,80000);                                  -- 参见㊺

-- 每单1条明细
INSERT INTO order_items(order_id,order_created_at,product_id,qty,price)
SELECT o.id, o.created_at,
       (SELECT id FROM products OFFSET floor(random()*3000) LIMIT 1), -- 参见㊼㊻
       (floor(random()*4)+1)::int,                                    -- 参见⑤①
       round((random()*200+5)::numeric,2)                              -- 参见㊻⑤①
FROM orders o;
